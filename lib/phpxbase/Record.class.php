...
class XBaseRecord
{
    ...
    function __construct($table, $recordIndex, $rawData=false) {
        ...
        if ($rawData && strlen($rawData)>0) {
        	...
        	foreach ($table->getColumns() as $column) {
        		$column_data = trim(substr($rawData,$column->getBytePos(),$column->getDataLength()), " ");
				$this->choppedData[$column->getName()]= $column_data;
        	}
    	} else {
	    	$this->inserted=true;
	    	$this->deleted=false;
    ...
    function getObject($columnObj)
    {
        switch ($columnObj->getType())
        {
            ...
            case DBFFIELD_TYPE_DATETIME : return $this->getDateTime($columnObj);break;
            case DBFFIELD_TYPE_FLOATING : return $this->getFloat($columnObj);break;
            case DBFFIELD_TYPE_LOGICAL :  return $this->getBoolean($columnObj);break;
            case DBFFIELD_TYPE_MEMO :     return $this->getMemo($columnObj);break;
            case DBFFIELD_TYPE_NUMERIC :  return $this->getInt($columnObj);break;
            case DBFFIELD_TYPE_INDEX :    return $this->getIndex($columnObj);break;
