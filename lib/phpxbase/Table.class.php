...
class XBaseTable
{
    ...
    function toHTML()
    {
	    $result = '<table class="dbf-dump" style="border:2px solid black;font-size:11px;">'."\n";
	    $result.= "<tr>\n";
	    foreach ($this->getColumns() as $i=>$c) {
		    $result.= '<th style="background:#cccccc;">'.$c->getName()."</th>\n";
	    }
	    $result.= "</tr>\n";
	    $this->moveTo(-1);
	    $tdArgs = '';
	    while ($row = $this->nextRecord())
	    {
		    ...
		    {
			    $result.= "<td $tdArgs >".$row->getString($col)."</td>\n";
		    }
		    $result.= "</tr>\n";
	    }
	    $result.= "</table>\n";
