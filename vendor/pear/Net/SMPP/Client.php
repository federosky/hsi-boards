...
class Net_SMPP_Client
{
     ...
     * @see  __construct()
     */
    function Net_SMPP_Client($host, $port, $timeout)
    {
    ...
    function &connect()
    {
        ...
        $res =& $this->_socket->connect($this->host, $this->port, null, $this->timeout);
        if (!PEAR::isError($res)) {
     ...
     * @return  void
     */
    function log($msg, $level =0 ) //= PEAR_LOG_CRIT
    {
    ...
    function &_commandStates()
    {
        static $states = array(
            ...
            'query_sm'              => array(NET_SMPP_CLIENT_STATE_BOUND_RX,
                                             NET_SMPP_CLIENT_STATE_BOUND_TX,
                                             NET_SMPP_CLIENT_STATE_BOUND_TRX),
